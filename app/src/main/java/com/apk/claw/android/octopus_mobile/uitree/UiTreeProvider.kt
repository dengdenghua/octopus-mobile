...
/** Shizuku 通道：基于 uiautomator dump，可补足 A11y 拿不到的系统对话框/自绘 UI（shell UID 2000） */
class ShizukuTreeProvider : UiTreeProvider {
    ...
    override fun getTree(full: Boolean): UiTree? {
        ...
        val root = parseUiautomatorXml(xml, w, h) ?: return null
        val tree = UiTree(
            root = root,
            screenWidth = w,
            screenHeight = h,
...
/**
 * Root 通道：与 [ShizukuTreeProvider] 同样基于 uiautomator dump，但走 [RootShellService]（root UID 0）。
 *
 * 优先级最低的 fallback —— A11y + Shizuku 都不可用时使用。
 * 能力等价 Shizuku 通道（同样的 uiautomator XML + 同样的 stableId 格式），
 * 但要求设备已 root（Magisk/KernelSU），不依赖 Shizuku 授权。
 *
 * stableId 与 ShizukuTreeProvider 完全一致（共用 [parseUiautomatorXml]），跨通道查找稳定。
 */
class RootTreeProvider : UiTreeProvider {
    override val source: String = "root"

    override fun isAvailable(): Boolean = RootShellService.isAvailable()

    override fun getTree(full: Boolean): UiTree? {
        // uiautomator dump 写到 /sdcard 临时文件 → cat 读出 → rm 清理
        // 三步都走 RootShellService 白名单（已扩 uiautomator dump / cat /sdcard/ / rm -f /sdcard/octopus_ui_dump_）
        val tempName = "octopus_ui_dump_${System.currentTimeMillis()}.xml"
        val tempPath = "/sdcard/$tempName"
        val dumpRes = RootShellService.exec("uiautomator dump $tempPath") ?: return null
        if (dumpRes.exitCode != 0) {
            XLog.w("RootTreeProvider", "uiautomator dump failed: ${dumpRes.stderr.take(200)}")
            return null
        }
        val xml = RootShellService.exec("cat $tempPath")?.stdout?.ifEmpty { null }
        runCatching { RootShellService.exec("rm -f $tempPath") }
        if (xml == null) return null

        // 屏幕尺寸：优先 wm size（root 可读真实物理尺寸），失败回退到 Resources.displayMetrics
        val (w, h) = parseWmSize(RootShellService.exec("wm size")?.stdout)
            ?: ClawApplication.instance.resources.displayMetrics.let { it.widthPixels to it.heightPixels }
        val root = parseUiautomatorXml(xml, w, h) ?: return null
        val tree = UiTree(
            root = root,
            screenWidth = w,
    ...
    /** 解析 `wm size` 输出 "Physical size: 1080x2400" → (1080, 2400) */
    private fun parseWmSize(stdout: String?): Pair<Int, Int>? {
        if (stdout.isNullOrBlank()) return null
        // 取最后一行 "Physical size: WxH" 或 "Override size: WxH"
        val sizeLine = stdout.lineSequence().lastOrNull { it.contains("size:") } ?: return null
        val m = Regex("(\\d+)x(\\d+)").find(sizeLine) ?: return null
 ...
 *
 * 优先级：A11y（最详细）→ Shizuku uiautomator（补足系统对话框，shell UID）→ Root uiautomator（root UID，最低 fallback）→ Remote（远程设备）
 *
 * 替代之前散落在 ClawAccessibilityService.performTap 等方法里的硬编码 fallback。
 * StableIdResolver / GetScreenInfoTool / TapTool 等所有调用方零改动自动获益。
 */
...
/**
 * 解析 uiautomator dump XML 为 UiNode 树。
 *
 * 抽为顶层函数供 [ShizukuTreeProvider] 和 [RootTreeProvider] 共用，
 * 保证两个通道产出的 stableId 格式完全一致（跨通道查找稳定）。
 *
 * stableId 格式：`{vid|class.simpleName}:{text.hashCode().hex}:{bounds.toShortString()}`
 */
fun parseUiautomatorXml(xml: String, screenW: Int, screenH: Int): UiNode? {
    val parser = XmlPullParserFactory.newInstance().newPullParser()
    parser.setInput(StringReader(xml))
    return parseXml(parser, screenW, screenH)
}
