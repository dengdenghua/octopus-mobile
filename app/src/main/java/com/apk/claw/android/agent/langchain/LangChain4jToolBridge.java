...
public class LangChain4jToolBridge {
    ...
    public static String executeToolRequest(ToolExecutionRequest request) {
        ...
        // 排除 imageBase64 以节省 token，LLM 无法在工具结果中解析 base64 图片
        Map<String, Object> resultForJson = new HashMap<>();
        resultForJson.put("isSuccess", result.isSuccess());
        resultForJson.put("data", result.getData());
        resultForJson.put("error", result.getError());
        return GSON.toJson(resultForJson);
    }
}
