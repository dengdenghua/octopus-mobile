...
class SettingsViewModel : ViewModel() {
    ...
    fun refresh() {
        ...
        val map = mapOf(
            ...
            MenuAction.OCTOPUS_RUNTIME.name to SettingValue.Text(getRuntimeTrailingText()),
    ...
    private fun getRuntimeTrailingText(): String {
        val url = KVUtils.getOctopusRpcUrl()
        if (url.isEmpty()) return ClawApplication.instance.getString(R.string.common_unconfigured)
        val state = com.apk.claw.android.appViewModel.connectionState.value
        return when (state) {
