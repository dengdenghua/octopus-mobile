...
public class ClawAccessibilityService extends AccessibilityService {
    ...
    protected boolean onKeyEvent(android.view.KeyEvent event) {
        ...
        // 转发给导航录制器（被动学习 + 主动录制）
        try {
            com.apk.claw.android.tool.impl.NavigateTool.Companion.getRecorder().onKeyEvent(event.getKeyCode());
        } catch (Exception e) {
            // 录制器未初始化时忽略
        }
        return false;  // 不消费按键
    ...
    public boolean performTap(int x, int y, long durationMs) {
        ...
        Boolean shellResult = ShizukuShellService.INSTANCE.tap(x, y);
        if (shellResult != null) {
            XLog.d(TAG, "Shizuku tap (" + x + "," + y + "): " + shellResult);
            return shellResult;
        }
    ...
    public boolean performLongPress(int x, int y, long durationMs) {
        ...
        Boolean shellResult = ShizukuShellService.INSTANCE.longPress(x, y, durationMs);
        if (shellResult != null) {
            XLog.d(TAG, "Shizuku longPress (" + x + "," + y + "," + durationMs + "ms): " + shellResult);
            return shellResult;
        }
        // Fallback: AccessibilityService dispatchGesture
        Path path = new Path();
    ...
    public boolean performSwipe(int startX, int startY, int endX, int endY, long durationMs) {
        ...
        Boolean shellResult = ShizukuShellService.INSTANCE.swipe(startX, startY, endX, endY, durationMs);
        if (shellResult != null) {
            XLog.d(TAG, "Shizuku swipe: " + shellResult);
            return shellResult;
        }
        // Fallback: AccessibilityService dispatchGesture
        Path path = new Path();
    ...
    public Bitmap takeScreenshot(long timeoutMs) {
        ...
        Bitmap shellBmp = ShizukuShellService.INSTANCE.screenshot();
        if (shellBmp != null) {
    ...
    public boolean sendKeyEvent(int keyCode) {
        ...
        Boolean shellResult = ShizukuShellService.INSTANCE.keyEvent(keyCode);
        if (shellResult != null) {
            XLog.d(TAG, "Shizuku keyEvent " + keyCode + ": " + shellResult);
            return shellResult;
        }
